...
{
    uint32_t mask = 1;
    for (uint8_t channel = 0; channel < DMA_NUM_CHANNELS; channel++) {
...
{
    dma_clear_interrupt(channel);

    *(dma_channel_cfg(channel)) = 0;
...
{
    return dma_cfg(channel)->DSR_BCR & DMA_DSR_BCR_DONE;
}
...
{
    return dma_cfg(channel)->DSR_BCR & (DMA_DSR_BCR_CE | DMA_DSR_BCR_BES | DMA_DSR_BCR_BED);
}
...
{
    dma_cfg(channel)->DCR |= DMA_DCR_CS; // DCR
